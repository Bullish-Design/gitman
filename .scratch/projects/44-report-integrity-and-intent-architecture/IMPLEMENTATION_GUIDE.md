...
- **`fix-reconcile-divergent-lane`** (`031165b`, unlanded, +99 −3) adds
  ...
  confirms it does not cover the shape that livelocked `devman`. Either land it for the shape it
  does fix, or abandon it and let Stage 3's registry subsume it. **Recommendation: abandon it.**
  Its approach is the pattern that produced the livelock.
...
| 3 | G2 | Typed anomalies; subject-scoped gate | large | 2 |
...
**Fault:** ISSUE.md §4. **Size:** large. This is the stage that retires the livelock *class*.
...
### 3.2 New module: `src/gitman/anomalies.py`

```python
@dataclass(frozen=True)
class Anomaly:
    kind: str                     # stable slug, e.g. "divergent-change-id"
    subject: Subject              # Lane(name) | Trunk() | Ref(name) | Workspace(name) | Change(id)
    detail: str                   # human-readable specifics
    blocks: frozenset[str]        # intent names this anomaly must block
    repair: Repair | None         # None = no automatic repair; `manual` explains what a human does
    manual: str | None = None
```

One registry maps `kind` → detector + repair. Populate it from the five checks already inside
`capture_state` (`state.py:437-712`):

| kind | current detector |
|---|---|
| `conflicted-trunk` | `_trunk_conflicted`, `state.py:465-487` |
| `conflicted-lane` | `_conflicted_lanes` |
| `stray-change` | `find_strays`, `state.py:24-36` |
| `divergent-change-id` | the H1/I5 scan, `state.py:617-630` |
| `colocated-ref-desync` | `colocated_ref_desync`, `state.py:637-660` |

Each row must declare `blocks` and either a `repair` or an explicit `manual`. **A row with
neither is a bug** — assert that at import time. That is how "detected but unfixable" becomes
visible instead of becoming a field incident.

### 3.3 Rewire the gate

`RepoState` gains `anomalies: list[Anomaly]`. Keep `canonical` / `off_canonical` as derived
properties so `status`, `doctor` and `render.py` keep working during the migration.

Each intent declares the subjects it touches. Start with a static map in `core.py` — a dict from
intent name to a function of its arguments — rather than threading a new parameter through 17
signatures.
...
blocking = [a for a in state.anomalies
            if intent in a.blocks and a.subject in subjects_touched]
if blocking:
    raise GitmanError(..., subject=str(blocking[0].subject),
                      remedies=[r for a in blocking if (r := a.repair_intent())])
```

### 3.4 Set `blocks` deliberately

This is the design work of the stage. Guidance:

- **`abandon` is blocked by almost nothing.** It is the escape hatch. Issue 42 §3a is the proof:
  when `abandon` refuses, the repository is sealed.
- **`reconcile` / `repair` is blocked by nothing.** It is the repair verb.
- **`status`, `log`, `doctor`** are reads and are never blocked.
- A **lane-scoped** anomaly blocks only intents acting on that lane or its descendants.
- A **trunk-scoped** anomaly blocks `land` and `push`, not `describe` or `split`.

### 3.5 Make the remedy verified

In Stage 2 `remedies` was hand-maintained. Now compute it: the renderer asks the registry which
intent repairs this anomaly kind, and asserts that intent is not itself blocked. Add a test:

> For every anomaly kind, the remedy it advertises is permitted in a repo exhibiting that
> anomaly.

That test makes issue 42's livelock unrepresentable.

### 3.6 Fold reconcile into the registry

`do_reconcile`'s four hand-rolled surveys become "run every registered repair whose detector
fires." Stage 1's honesty fix then falls out for free: reconcile's coverage *is* the predicate,
because both read the same registry.

If you kept `fix-reconcile-divergent-lane`, fold its
`find_unbookmarked_divergent_lane_commits` in here as one more detector for the
`divergent-change-id` kind — **and** add the bookmark-vs-remote shape it misses, resolved by the
existing `try_merge` content classifier (`state.py:153-182`, issue 42 D2). That classifier
already distinguishes a re-hash twin from a genuine fork; it is applied to trunk-vs-origin today
and not to lane-vs-its-own-origin, which is the identical question.
...
- Every registry row has a repair or an explicit `manual`.
- The remedy-is-permitted test passes.
...
### 7.2 Kill the prose coupling

`render.py:96-98` picks a recovery hint by substring-matching prose composed in `state.py`:
...
Once Stage 3's typed anomalies exist, replace this with a match on `anomaly.kind`. Reword a
message today and the renderer silently picks the wrong branch, and no test can catch it.
