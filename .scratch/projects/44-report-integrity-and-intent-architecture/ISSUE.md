...
| G2 | Typed anomalies with subjects; one detect/repair registry; subject-scoped gate; verified `remedies`. **DESIGN RESOLVED (2026-09-16)** — see `IMPLEMENTATION_GUIDE.md` §3, sub-stages 3a-3d. Taxonomy is **7 kinds, not 5**; blocking is decided by safety, not repair availability; the postcondition must become delta-based or the stage silently self-reverts. | new `anomalies.py`, `state.py`, `reconcile.py`, `invariants.py`, `render.py` | high |
