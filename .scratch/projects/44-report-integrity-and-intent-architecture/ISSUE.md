...
**Date:** 2026-09-16
**Status: CLOSED 2026-09-17 (project 46, stage S8).** All nine goals, G0–G8, shipped. G0 (reconcile honesty) · G1 (refusals render through `_refusal_result`) · G2 (typed anomaly registry, one repair path, delta-based postcondition) · G3 (per-session path fingerprint) · G4 (git refs a publication artifact; `+` the lane-path separator) · G5 (verb consolidation behind hidden aliases; `workspace` noun) · G6 (`Plan` as a value + generic `--dry-run`) · G7 (lane lifecycle facts: `created_at`/`updated_at`, `merged`; `landed` deleted) · G8 (this concept doc rewritten from the code, with a drift test). Deliberately left: `sync --trunk` (the old `pull`) stays on the callback executor, because it runs a trial merge as planning input and every recovery path leans on it; and issue 33's history ledger is unbuilt, because G7's derivation removed the need for it.
...
| **G1** | ~~**One closed `Outcome` sum type returned by every intent.**~~ **PARTIALLY SHIPPED (2026-09-16, Stage 2).** Every `GitmanError` now renders through `cli.py:main()`'s `_refusal_result` as an `IntentResult` with `outcome="REFUSED"` — same renderer, same `--json` shape as a success. This closes issue 43 D1: refusals print `Gitman <verb> — REFUSED` and are never silent. Not done: `outcome` is still a bare `str`, not a closed enum (§2.5/Step E of the guide), and the 101 call sites are not individually enriched with `subject`/`remedies` (Step F) — the fields exist on `GitmanError` and flow through, but are mostly unset. Replace 101 `raise GitmanError` with `Declined`. | `models.py`, `core.py`, `cli.py`, `render.py` | **highest** |
| G2 | ~~Typed anomalies with subjects; one detect/repair registry; subject-scoped gate.~~ **SHIPPED (2026-09-17, Stages 3a-3f).** `bb11960` typed registry, `canonical`/`off_canonical` derived from it · `252a9ea` subject-scoped gate, `abandon` ungated · `52b4d5f` `render_status` matches kind, not prose · `43ef935` content-aware `lane-divergent` repair · `ee61e0f` repair hygiene (one `adopted-<commit>` minter, one `capture_state` per verb, one `ContentRelation`) · `6f3746e` `repairs.REPAIRS` dispatches, guarded by an import-time two-way assertion. Taxonomy is **9 kinds** (`anomalies.REGISTRY`), not the 5 or 7 earlier drafts assumed; blocking is decided by safety, not repair availability; the postcondition is delta-based. Not done: `remedies` are not yet verified against the gate (that rides on G6's `Plan`). | new `anomalies.py`, new `repairs.py`, `state.py`, `reconcile.py`, `invariants.py`, `render.py`, `models.py`, `lanes.py` | high |
| G3 | ~~Per-invocation path fingerprint; `start`/`describe` report and scope by provenance~~ **SHIPPED (2026-09-17, project 46 S4).** New `provenance.py`: a per-session (identity = `GITMAN_SESSION` or workspace name) path fingerprint under `.gitman/session-paths.json`, written atomically on every command that snapshots and pruned by age. `RepoState.foreign_paths`/`session_identity` carry "paths in `@` this session did not write"; `render_status` renders them; `start` adopts/refuses per the fingerprint and reports the counts (`--adopt-all`/`--adopt-mine`); `save` reports (it cannot restrict — jj already snapshotted `@`). Advisory by decision (D-C2): a missing record degrades to the old behaviour with a note. Fixes issue 43 D4 (build the lane on the dirty `@`, never beside it). | new `provenance.py`, `session.py`, `state.py`, `models.py`, `render.py`, `core.py`, `cli.py` | high |
| G4 | ~~Git refs become a publication artifact: export on demand, never gate on them, total ref encoding~~ **SHIPPED** — stages 4a-4d, 4e (2026-09-17, project 46 S2 — `doctor`'s `colocated-index` row, `state.intent_to_add_entries`), 4f (2026-09-17, project 46 S3 — D-A2, `+` the lane-path separator everywhere; see `SCOPING.md` §2.2) | `session.py`, `invariants.py`, `state.py`, `lanes.py`, `anomalies.py`, `repairs.py` | high |
| G5 | ~~Verb consolidation behind deprecating aliases; `workspace` promoted to a noun~~ **SHIPPED (2026-09-17, project 46 S6).** Renamed behind hidden, warning aliases: `save`→`describe`, `reconcile`→`repair`, `subtask`→`start` (qualifying form). `sync` now absorbs `pull` (`sync --trunk`) and `catchup` (`sync --trunk --all`); `push` stays. New `workspace list`/`forget`/`prune` subgroup, plus a `status` note for laneless registrations. Aliases forward every option and exit code and name the replacement in the report's notes (`--json` too). The visible verb surface is now 21, down from 24. | `cli.py`, `core.py`, `repair.py`, `repairs.py`, `anomalies.py`, `state.py`, `render.py`, `doctor.py`, `invariants.py` | medium |
| G6 | ~~`Plan` as a value; generic gate, wrapper, undo line, `--dry-run`, partial-progress~~ **SHIPPED (2026-09-17, project 46 S7).** New `plan.py` (`Plan`/`Step` values + `describe_plan`) and the `run_plan` executor beside `canonical_tx`/`canonical_guard`. `describe`, `switch`, `start`, `split` and `land` migrated one per commit; each accepts `--dry-run` (generic handler in `cli.py`, no mutation). `Plan.postcondition` is additive to the global delta check (D-D2). `land` now records **one batch undo checkpoint** per invocation — a single `gitman undo` rewinds every lane it landed, closing both `core.py` S9a TODOs. `pull` deliberately does NOT migrate (it runs a trial merge as planning input, and every recovery path leans on it). `abandon --recursive` and `pull` keep their own per-node checkpoints: they are outside the guide's migration list, and the per-node undo is asserted behaviour (`test_abandon_recursive_undo_reverses_one_node_at_a_time`). | new `plan.py`, `invariants.py`, `state.py`, `core.py`, `cli.py` | medium |
...
| G8 | ~~Rewrite `GITMAN_CONCEPT.md` from the code; add a test asserting the CLI verb list matches the doc's intent table~~ **SHIPPED (2026-09-17, project 46 S8).** §5/§6/§7/§11 rewritten from the shipped code (invariants, the `Plan` executor with `sync --trunk` deliberately excluded, the 25-row verb table incl. the `remote`/`workspace` group rows, delta-based rollback, note-only kinds, refs as a publication artifact, the post-guard network call). New `tests/test_concept_doc_drift.py` asserts §7's table equals the shipped command set both directions (groups handled generically, a zero-row parse fails loudly), no shipped verb is deferred, and the how-to docs name no deprecated verb. **Closes issue 44.** | `docs/`, `tests/` | medium |
